...
- Changes in the effect can occur due to outside variables
- In cases where an effect is being studied over time, it is important to have a control group to compare the results to in order to have a baseline
- Sometimes, there either isn’t a control group or there is some bias introduced into the control group that makes the control group inadequate
- The control and experimental group should be as similar as possible, the only difference being the variable that is being manipulated
